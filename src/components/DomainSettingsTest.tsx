...
interface DomainTestResult {
	...
	settings: {
		...
		themes: {
			...
			sale: { start: string; end: string };
		};
	};
}

interface DomainTestError {
	error: string;
}
						...
						{isDomainTestError(testResult) ? (
							...
							<>
								...
								<div className="flex justify-between items-center">
									...
									<Badge
										variant={
											testResult.hasCustomization ? "default" : "outline"
										}
									>
								...
								<div className="space-y-2">
									...
									<div className="text-xs space-y-1">
										<div>
											Font: <strong>{testResult.settings.font}</strong>
										</div>
										<div>
											Design Type:{" "}
											<strong>{testResult.settings.designType}</strong>
										</div>
										<div>
											Show Theme Labels:{" "}
											<strong>
												{testResult.settings.showThemeLabels ? "Yes" : "No"}
											</strong>
										</div>
									</div>
								</div>
