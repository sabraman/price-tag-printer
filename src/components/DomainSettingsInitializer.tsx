...
export function DomainSettingsInitializer() {
	const { settings, hasCustomization, isHydrated, configName } =
		useDomainSettings();
	...
	const setShowThemeLabels = usePriceTagsStore(
		(state) => state.setShowThemeLabels,
	);
	...
	useEffect(() => {
		...
		if (hasCustomization && isStoreInInitialState) {
			...
			console.log(
				`🌐 Domain settings available for ${configName} but store already initialized`,
			);
			hasInitialized.current = true;
		} else {
 ...
 */
export function DomainSettingsDebug({
	enabled = false,
}: {
	enabled?: boolean;
}) {
	const { hostname, configName, hasCustomization, settings, isHydrated } =
		useDomainSettings();
