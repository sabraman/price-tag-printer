...
export function StaticPriceTagsPage({
  ...
  const validThemeTypes = [
    'default', 'new', 'sale', 'white', 'black', 'sunset', 'ocean',
    'forest', 'royal', 'vintage', 'neon', 'monochrome', 'silver',
    'charcoal', 'paper', 'ink', 'snow'
  ];

  const getThemeForItem = (item: any) => {
    const itemDesignType = useTableDesigns && item.designType 
      ? item.designType 
      : designType;
    
    const safeDesignType = itemDesignType && validThemeTypes.includes(itemDesignType)
      ? itemDesignType
      : 'default';
  ...
  const shouldShowLabel = (itemDesignType: string) => {
    return showThemeLabels && (itemDesignType === 'new' || itemDesignType === 'sale');
  };
  ...
  const StaticPriceTag = ({ item }: { item: any }) => {
    ...
    const cutLineColor = !cuttingLineColor || cuttingLineColor === '#cccccc'
        ...
        >
          <defs>
            {isGradient && (
              <linearGradient 
                id={gradientId}
                x1="13.75" y1="108.41" x2="148.83" y2="10.42" 
          ...
          <rect 
            ...
            strokeWidth={shouldShowBorder ? (itemDesignType === 'white' ? 1.5 : 1) : 0}
          />
          
          {/* Cutting lines */}
          <line 
            x1="0" y1="0" x2={originalWidth} y2="0"
            stroke={cutLineColor}
            strokeWidth="0.75"
            strokeDasharray="12,3"
          />
          <line 
            x1="0" y1={originalHeight} x2={originalWidth} y2={originalHeight}
            stroke={cutLineColor}
            strokeWidth="0.75"
            strokeDasharray="12,3"
          />
          <line 
            x1="0" y1="0" x2="0" y2={originalHeight}
            stroke={cutLineColor}
            strokeWidth="0.75"
            strokeDasharray="8,3"
          />
          <line 
            x1={originalWidth} y1="0" x2={originalWidth} y2={originalHeight}
            stroke={cutLineColor}
            strokeWidth="0.75"
            strokeDasharray="8,3"
          />
          ...
          {hasMultiTier ? (
            <div style={{
              ...
              <div style={{ 
                ...
                <div style={{ fontSize: '12px', color: theme.textColor, fontFamily: font, fontWeight: 'bold' }}>
                  от 3 шт.
                </div>
                <div style={{ fontSize: '46px', color: theme.textColor, fontFamily: font, fontWeight: 'bold' }}>
              ...
              <div style={{ 
                ...
                <div style={{ fontSize: '32px', color: theme.textColor, fontFamily: font, fontWeight: 'bold' }}>
              ...
              <div style={{ 
                ...
                <div style={{ fontSize: '24px', color: theme.textColor, fontFamily: font, fontWeight: 'bold' }}>
    ...
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      {chunkedItems.map((chunk, pageIndex) => (
        <div
          ...
          style={{
            pageBreakAfter: pageIndex < chunkedItems.length - 1 ? 'always' : 'auto',
          }}
        >
